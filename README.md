...
NCP_CLIENT_ID=...                   # Naver Cloud Platform 클라이언트 ID
NCP_CLIENT_SECRET=...               # Naver Cloud Platform 시크릿
...
[NCP 콘솔](https://console.ncloud.com) → Application Services → Maps에서 아래 서비스를 모두 활성화해야 합니다:

| 서비스 | 용도 |
|--------|------|
| Web Dynamic Map | 브라우저 지도 렌더링 |
| Geocoding | 장소명 → 좌표 변환 (서버사이드) |
| Directions 15 | 도로 이동 경로 |

Web Dynamic Map 서비스 URL에 `http://localhost:3000` 등록 필요.
...
location 문자열
  ...
  → Naver Geocoding API (지역 기준) — coordinate 파라미터로 목적지 근처 결과 우선 정렬
  → null이면 Naver Geocoding API (전국)
  → null이면 쿼리 단순화 변형 재시도 (Naver 지역/전국 각각)
  → null이면 Nominatim (OSM) — Naver 장애 대비 순차 fallback
  → 그래도 null이면 coords = undefined (지도 마커 없이 목록에만 표시)

모든 Naver 호출을 Promise.all로 병렬 처리 (~2-3초)
Naver 전체 실패 시 Nominatim 순차 fallback (~27초, Naver 장애 대비)
```

**Naver Geocoding 선택 이유:**
- 기존 `NCP_CLIENT_ID` / `NCP_CLIENT_SECRET` 그대로 사용 (별도 키 불필요)
- Kakao REST API 키 의존 제거
- 한국 주소/POI 커버리지 우수
